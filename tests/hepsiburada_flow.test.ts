...
test('Hepsiburada Hedef Odaklı Doğrulama Testi', async ({ page, context }) => {
    ...
    // 3. Sepete Ekle
    await productPage.locator('[data-test-id="addToCart"]');
    ...
    await expect(cartItem).toBeVisawait page1.locator('div').filter({ hasText: /^Üye olmadan devam et$/ }).nth(1).click();
    await page1.locator('div').filter({ hasText: /^Üye olmadan devam et$/ }).nth(1).click();ible();
...
await page1.getByRole('button', { name: 'Sepete git' }).click();
...
await page1.locator('[data-test-id="add-to-cart"]').click();
await page1.locator('.checkoutui-Modal-muJqBya3N8yo8EA2G9s5').click();
await page1.getByRole('button', { name: 'Sepete git' }).click();
await page1.locator('div').filter({ hasText: /^Alışverişi tamamla$/ }).click();
await page1.locator('div').filter({ hasText: /^Üye olmadan devam et$/ }).nth(1).click();